...
func (this *Ivoix) ParseReDo() {
	...
	for iter.Next(&audio) {
		req := request.NewRequest(mp3Url).SetMethod("POST").
			...
			AddPostParam("aid", audio.Aid).AddPostParam("uname", "lsvking").AddCallParam("aid", audio.Aid).AddCallParam("name", audio.Name).
...
func (this *Ivoix) ParseMp3(response *http.Response, params map[string]string) {
	...
	if len(filePath) > 200 {
		...
		req := request.NewRequest(mp3Url).SetMethod("POST").
			...
			AddPostParam("aid", params["aid"]).AddPostParam("uname", "13264180593").AddCallParam("aid", params["aid"]).AddCallParams(params).AddCallParam("name", params["name"]).
			SetCallback("ParseMp3")
